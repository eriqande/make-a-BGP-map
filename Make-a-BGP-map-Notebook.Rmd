...
color in each cluster is at maximum opacity at the highest value within the clusters, etc.
...
we just want to get the boundary lines that are internal to the coast.  Otherwise you get
a think line around the coast that is not so great...
```{r, eval=FALSE}
# country boundaries
tmpfile <- tempfile()
downloader::download(
...
The new version of ggplot makes it super easy to project everything on the fly
by passing the projection to `coord_sf()`.   For things in North America,
it seems that a Lambert conic project does a nice job of keeping British Columbia
...
Seeing at sort of small on the screen does not really do justice to it.  You can `ggsave` it
...
While that looks nice, some people, like Kristen, might prefer to have the
background part there in a rectangle, rather than a conic surface.  We might
be able to do that be determining the xlim and ylim to `coord_sf()`.  But, note that
...
# and it is stupid how it prints in the notebook, so
...
Okay, that does it.  In reality, you could want to expand the original domain
and then cut stuff off, leaving a little more room.  

## A final note

If you want to add more elements to the figure you can do so by giving them
a lat-long and then putting them there using geom_sf().  It all fits nicely
